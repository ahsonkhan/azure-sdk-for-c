...
static az_result az_span_reader_skip_json_white_space(az_span* self)
{
  ...
  while (az_json_is_white_space(*self.ptr[0]))
  {
...
{
  // skip '"'
  int32_t reader_initial_length = *self.size;
  uint8_t* p_reader = *self.ptr;
  while (true)
  {
    uint32_t ignore = { 0 };
    ...
    switch (result)
    {
      ...
      {
        int32_t read_count = reader_initial_length - *self.size;
    ...
    az_json_token* out_token)
{
  ...
  if (p_state->_internal.reader.size > 0)
  {
...
az_json_parser_parse_token(az_json_parser* json_parser, az_json_token* out_token)
{
  ...
  bool const is_empty = *p_reader.size == 0; // everything was read
...
{
  az_span* p_reader = &json_parser->_internal.reader;
  uint8_t const c = *p_reader.ptr[0];
  if (c == ',')
  {
...
AZ_NODISCARD az_result az_json_parser_done(az_json_parser* json_parser)
{
  ...
  if (json_parser->_internal.reader.size > 0
